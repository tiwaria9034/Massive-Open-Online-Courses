"""
Code to implement Bayes Classifier using Gaussian Mixture Model (GMM).
"""
...
class BayesClassifier:
    ...
    def fit(self, X, y):
        ...
        for class_idx in range(self.num_classes):
            ...
            gmm = BayesianGaussianMixture(n_components=self.num_classes)
            gmm.fit(X_class)

            self.gaussians[class_idx] = gmm
    ...
    def sample_given_y(self, y):
        ...
        # As per the API, the sample method would return a randomly generated sample along with
        # the label of the Gaussian in the Mixture Model to which this sample belongs. We will
        # use the return label to arrive at the Gaussian for this random sample.
        sample = gmm.sample(n_samples=1)
        mean = gmm.means_[sample[1]]
        return clamp_sample(x=sample[0]), mean
...
def main():
    ...
    for class_idx in range(clf.num_classes):
        ...
        sample, mean = clf.sample_given_y(y=class_idx)
